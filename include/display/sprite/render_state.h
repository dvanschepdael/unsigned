...
typedef struct USpriteRenderState {
    ...
    /*
     * CPU-side transform prepared during active display and consumed after VBlank.
     * Keeping this tiny cache in the sprite state removes effect sampling, viewport
     * projection and transform change detection from the critical commit window.
     */
    s16 prepared_screen_x;
    s16 prepared_screen_y;
    s16 prepared_draw_x;
    s16 prepared_draw_y;
    u16 prepared_scb2;
    bool prepared_valid;
    bool prepared_hidden;
    bool prepared_per_column;
} USpriteRenderState;
