...
/** Camera-space world rectangle cached for repeated culling during one subsystem pass. */
typedef struct UViewportWorldBounds {
    s32 left;
    s32 top;
    s32 right;
    s32 bottom;
} UViewportWorldBounds;
 ...
 * @pre `viewport`, its camera and `bounds` are valid; viewport dimensions are non-zero.
 */
 ...
 * @pre `viewport` has a bound camera.
 */
 ...
 * @pre `viewport` has a bound camera and non-zero dimensions.
 * @pre Width/height are positive; margins are symmetric and non-negative.
 */
