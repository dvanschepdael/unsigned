...
struct UAudioManager {
    /** Caller-owned event definitions; must outlive the manager while installed. */
...
/** Reset runtime state; no event definitions are installed until `unsigned_audio_manager_set_events()`.
 * @pre `audio` is valid.
 */
 ...
 * @pre `audio` is valid and `seed != 0`.
 */
void unsigned_audio_set_random_seed(UAudioManager *audio, u16 seed);

/** Queue one gameplay SFX command.
 * @return false only when the SFX queue is full.
 * @pre `audio` is valid and `command` is in the game command range.
 */
