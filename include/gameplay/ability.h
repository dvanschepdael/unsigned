...
typedef struct UGameplayAbilityContainer {
    /** Number of immutable ability definitions in `instances`. */
    u8 count;
 ...
 * @return Matching ability definition, or NULL when the catalog is empty or the tag is absent.
 * @pre `abilities` is valid and `tag != UNSIGNED_GAMEPLAY_TAG_NONE`.
 */
 ...
 * @return true when no active owner tag appears in `blocked_by_tags`; false when a blocker matches.
 * @pre `tags` and `ability` are valid.
 */
