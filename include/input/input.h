...
typedef enum UInputMatch {
    /** Every button in the binding mask must match. */
    U_INPUT_MATCH_ALL = 0,
 ...
 * @pre `input` is valid and `player_count` is in `1..U_INPUT_PLAYER_CAPACITY`.
 */
 ...
 * @pre `controller` is valid and `hold_frames > 0`.
 */
void unsigned_input_controller_tick(UInputController *controller, UInputMask buttons_down, u8 hold_frames);

/**
 * @brief Converts held D-pad input into a signed {-1,0,1} direction; opposite directions cancel.
 * @pre `input` is valid.
 */
