...
/**
 * Configure the direct-storage API.
 * @pre `config` is valid and `config->storage` is one of the declared `UStorage` values.
 */
...
/**
 * Report whether one physical slot contains an Unsigned record.
 * @pre `slot < UNSIGNED_SAVE_SLOT_COUNT` and `out_exists` is valid.
...
/**
 * Persist one versioned record.
 * @pre `slot < UNSIGNED_SAVE_SLOT_COUNT`.
 * @pre `size <= UNSIGNED_SAVE_MAX_DATA_SIZE` and `data` addresses at least `size` readable bytes.
 */
USaveState unsigned_storage_write(u8 slot, const void *data, u16 size);
/**
 * Read and validate one versioned record.
 * @pre `slot < UNSIGNED_SAVE_SLOT_COUNT`.
 * @pre `data` addresses `capacity` writable bytes and `out_size` is valid.
 */
