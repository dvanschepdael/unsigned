...
/** Return true when the four-byte Unsigned record magic is present. @pre `record` addresses a full record. */
bool unsigned_save_record_has_magic(const u8 *record);

/** Build one complete record. @pre `record` is writable, `size` fits, and `data` addresses `size` readable bytes. */
void unsigned_save_record_build(u8 record[UNSIGNED_SAVE_RECORD_SIZE], const void *data, u16 size, u16 data_version);

/** Validate external record metadata/CRC/version, then copy the payload. @pre caller buffers satisfy their declared capacities. */
/** @pre `record`, `data` and `out_size` are valid. */
