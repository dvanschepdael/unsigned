...
/**
 * Return the consecutive physical-slot count required by the set.
 * @pre `set` is valid, `set->size` is in 1..UNSIGNED_SAVE_SET_MAX_DATA_SIZE, and the resulting
 *      range beginning at `first_slot` fits in `UNSIGNED_SAVE_SLOT_COUNT`.
 */
u8 unsigned_set_block_count(const UDataSet *set);

/**
 * A set exists only when every block belongs to the same complete generation.
 * @pre `set` satisfies `unsigned_set_block_count()` and `out_exists` is valid.
 */
 ...
 * @pre `set` describes a valid slot range and `set->data` addresses `set->size` bytes.
 */
USaveState unsigned_set_write(const UDataSet *set);

/** Validate every externally stored block in staging memory before modifying caller-owned data. */
