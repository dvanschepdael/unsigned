...
typedef struct UGameplayAbilityBinding {
    ...
    UInputMatch match;
    void *args;
    /** Preferred ability identity: resolved from `character->abilities` when non-zero. */
    UGameplayTag ability_tag;
    /** Optional direct definition for callers that do not use a tagged character catalog. */
