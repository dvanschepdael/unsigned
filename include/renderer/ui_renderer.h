/**
 * @file ui_renderer.h
 * @brief Neo Geo implementation of the generic UI renderer.
 */

#ifndef UNSIGNED_RENDERER_UI_RENDERER_H
#define UNSIGNED_RENDERER_UI_RENDERER_H

#include "display/ui/renderer.h"
#include "display/ui/types.h"

#define UNSIGNED_NEO_GEO_UI_TEXT_CAPACITY 64u

/* Shared contract with setup/build-progress-bar.py: six frame parts, nine fill widths each. */
#define U_NEO_GEO_UI_PROGRESS_VARIANTS 9u
#define U_NEO_GEO_UI_PROGRESS_TILE_COUNT 54u
#define U_NEO_GEO_UI_PROGRESS_INSET_LEFT 1u
#define U_NEO_GEO_UI_PROGRESS_INSET_RIGHT 1u

/**
 * @brief Neo Geo FIX-renderer configuration.
 *
 * @details `progress_bar_tile_base` selects 54 precomposed tiles: top left, top center,
 * top right, bottom left, bottom center, bottom right, each with fill widths 0..8.
 * Use the base generated by setup/build-progress-bar.py. Zero disables progress bars.
 * @invariant `selector_option_width > 0`.
 * @invariant A non-zero `progress_bar_tile_base` leaves room for `U_NEO_GEO_UI_PROGRESS_TILE_COUNT` tiles in FIX tile space.
 * @invariant Progress bars rendered by this backend keep the fixed `U_UI_PROGRESS_BAR_HEIGHT` and a width of at least two FIX tiles.
 * The caller loads the artwork palette into the slot selected by each widget.
 */
typedef struct UNeoGeoUIRendererConfig {
    UUIStyleId tall_label_style;
    u8 palette;
    u8 selector_option_width;
    u16 progress_bar_tile_base;
} UNeoGeoUIRendererConfig;

typedef struct UNeoGeoUIRenderer {
    UNeoGeoUIRendererConfig config;
    char text[UNSIGNED_NEO_GEO_UI_TEXT_CAPACITY];
} UNeoGeoUIRenderer;

/**
 * @brief Initializes the Neo Geo UI renderer to a valid empty runtime state.
 *
 * @param renderer Renderer abstraction/state used to draw the logical presentation.
 * @param backend Neo Geo-specific UI backend state bound to the generic renderer callbacks.
 * @param theme Theme containing the style callbacks/assets used by UI rendering.
 * @param config Neo Geo UI style/palette/tile configuration copied into `backend`.
 * @pre `renderer`, `backend`, and `config` are valid and `config` satisfies its tile-range invariant.
 */
void unsigned_neo_geo_ui_renderer_init(UUIRenderer *renderer, UNeoGeoUIRenderer *backend, const UUITheme *theme, const UNeoGeoUIRendererConfig *config);

#endif
