...
void unsigned_game_instance_prepare_render(UGameInstance *game) {
    if (game != NULL) {
        (void)unsigned_level_renderer_prepare(&game->renderer, &game->level, &game->viewport);
    }
}
...
void unsigned_game_instance_render(UGameInstance *game) {
    ...
    if (!unsigned_level_renderer_has_prepared_frame(&game->renderer)) {
        /* Preserve the old public API for callers that do not use the split frame runtime. */
        (void)unsigned_level_renderer_prepare(&game->renderer, &game->level, &game->viewport);
    }
    unsigned_level_renderer_commit(&game->renderer);
}

const URenderFrameStats *unsigned_game_instance_render_stats(const UGameInstance *game) {
    return game != NULL ? unsigned_level_renderer_stats(&game->renderer) : NULL;
}
