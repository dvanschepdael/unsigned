...
bool unsigned_audio_play_sound_effect(UAudioManager *audio, USoundCommand command) {
    if (audio->pending_sound_effect_count >= UNSIGNED_AUDIO_SFX_QUEUE_CAPACITY) {
        return false;
    }
...
static USoundCommand audio_pop_sound_effect(UAudioManager *audio) {
    if (audio->pending_sound_effect_count == 0u) {
        return U_AUDIO_COMMAND_NONE;
    }

    const USoundCommand command = audio->pending_sound_effects[audio->pending_sound_effect_head];
