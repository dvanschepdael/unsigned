...
static bool collision_projectile_hits_actor(const UCollisionActorIndex *actors, UPhysicsCollisionMask target_mask, const UCollisionBox *hitbox, const UActor *projectile_actor) {
    ...
    for (u8 i = range.first; i < range.end; ++i) {
        ...
        if (actor->collision.hitbox_channel != U_COLLISION_CHANNEL_NONE && (target_mask & (UPhysicsCollisionMask)(1u << hitbox_channel)) != 0u) {
            if (unsigned_physics_collision_box_intersects(hitbox, &actor->collision.hitbox)) {
                return true;
            }
        }

        if (actor->collision.hurtbox_channel != U_COLLISION_CHANNEL_NONE && (target_mask & (UPhysicsCollisionMask)(1u << hurtbox_channel)) != 0u) {
            if (unsigned_physics_collision_box_intersects(hitbox, &actor->collision.hurtbox)) {
                return true;
            }
        }
    }
...
void unsigned_collision_projectiles_resolve(UCollisionManager *collisions, const UCollisionActorIndex *actors, UPoolInstanceContainer *projectiles, const UTLSS *tlss) {
    ...
    for (u8 i = 0u; i < unsigned_pool_iteration_end(projectiles) && remaining > 0u; ++i) {
        ...
        const UPhysicsCollisionMask actor_channels = (UPhysicsCollisionMask)(actors->hitbox_channels | actors->hurtbox_channels);
        UPhysicsCollisionMask actor_mask = (UPhysicsCollisionMask)(collision_mask & actor_channels);
