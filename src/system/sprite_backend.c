...
static void sprite_backend_note_graphics_write(const USprite *sprite) {
    ...
    for (u8 column = 0u; column < sprite->render.sprite_count; ++column) {
        USpritePaddingState *state = &sprite_padding_state[sprite->render.first_sprite + column];
...
bool unsigned_sprite_backend_padding_is_valid(const USprite *sprite, u16 first_sprite) {
    ...
    if (sprite == NULL || sprite->definition == NULL || !unsigned_sprite_range_is_valid(first_sprite, sprite->render.sprite_count)) {
        return false;
    }
...
static void sprite_backend_write_unused_rows(const USprite *sprite) {
    ...
    unsigned_neogeo_vram_set_mod(1u);
    for (u8 column = 0u; column < sprite->render.sprite_count; ++column) {
        const u16 hardware_sprite = (u16)(sprite->render.first_sprite + column);
        *REG_VRAMADDR = (u16)(ADDR_SCB1 + hardware_sprite * 64u + (u16)definition->height_tiles * 2u);
...
void unsigned_sprite_backend_break_chain(u16 hardware_sprite) {
    if (hardware_sprite < UNSIGNED_SPRITE_FIRST || hardware_sprite > UNSIGNED_SPRITE_LAST) {
        return;
    }
