...
static bool neo_geo_bios_forced_start_handoff_pending(void) {
     ...
     * COIN_SOUND path.
     */
    return bios_user_request == (u8)U_NEO_GEO_BIOS_REQUEST_TITLE || bios_compulsion_timer_over == 0u || bios_compulsion_timer != 0u || unsigned_neo_geo_player_credits(U_NEO_GEO_PLAYER_1) != 0u ||
           unsigned_neo_geo_player_credits(U_NEO_GEO_PLAYER_2) != 0u;
}
...
void neo_geo_bios_process_start(void) {
    ...
    if (neo_geo_runtime_binding_state == U_NEO_GEO_RUNTIME_READY) {
        accepted = candidates;
