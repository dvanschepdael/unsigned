...
static void zoom_sample(UEffectSample *sample, u8 column, u8 column_count, u8 phase, const void *context) {
    ...
    sample->scale_x = zoom_lerp_q8(zoom->min_scale_x, zoom->max_scale_x, progress);
    sample->scale_y = zoom_lerp_q8(zoom->min_scale_y, zoom->max_scale_y, progress);
    sample->pivot_x = zoom->pivot_x;
    sample->pivot_y = zoom->pivot_y;
}
