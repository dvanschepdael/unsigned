...
static void transform_sample(UEffectSample *sample, u8 column, u8 column_count, u8 phase, const void *context) {
    ...
    sample->scale_x = unsigned_effect_clamp_q8(transform->scale_x);
    sample->scale_y = unsigned_effect_clamp_q8(transform->scale_y);
    sample->pivot_x = unsigned_effect_clamp_q8(transform->pivot_x);
    sample->pivot_y = unsigned_effect_clamp_q8(transform->pivot_y);
    if (transform->flip_x) {
...
void unsigned_effect_transform_set_scale(UTransformEffect *transform, u16 scale_x, u16 scale_y, u16 pivot_x, u16 pivot_y) {
    ...
    transform->scale_x = unsigned_effect_clamp_q8(scale_x);
    transform->scale_y = unsigned_effect_clamp_q8(scale_y);
    transform->pivot_x = unsigned_effect_clamp_q8(pivot_x);
    transform->pivot_y = unsigned_effect_clamp_q8(pivot_y);
}
