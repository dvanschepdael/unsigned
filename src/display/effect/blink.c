...
static void blink_sample(UEffectSample *sample, u8 column, u8 column_count, u8 phase, const void *context) {
    ...
    const u16 configured_cycle = (u16)blink->visible_phase + blink->hidden_phase;
    const u16 cycle = configured_cycle > 256u ? 256u : configured_cycle;
    (void)column;
    (void)column_count;

    if (cycle == 0u || blink->hidden_phase == 0u) {
        return;
    }
