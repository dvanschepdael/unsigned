...
`unsigned_game_instance_init()` receives already allocated buffers through `UGameInstanceStorage`. Capacity/storage relationships are authored construction contracts: size them from the project composition macros instead of adding dynamic allocation or defensive hot-path recovery.
...
- abilities;
- tags;
- generic ground-plane movement and orientation;
...
Beat'Em Up camera
    |
shared actor ordering
    |
collision probe / conditional materialization + hit/projectile detection
    |
level resolve_hits
    |
effects
...
- `unsigned_character_move(character, direction, horizontal_speed, vertical_speed)` moves each axis from the sign of `direction`, saturates coordinates to `s16`, and updates horizontal facing;
- `unsigned_character_set_facing()` is used when orientation must change without movement;
- `unsigned_physics_movement_constrain()` optionally applies `UMovementBounds` after movement.

The demo follows this pipeline in both `src/characters/player/demo/player_move.c` and Arthur's air steering in `src/characters/player/arthur/arthur_jump.c`: movement-speed attributes feed `unsigned_character_move()`, then level-owned bounds are applied separately. The old demo-specific movement helper is no longer part of the design.
...
5. let `engine/level/level_ai.c` and TLSS manage its cadence according to activity.
...
Loading is a direct replace operation over authored content: the previous level is unloaded first, then backgrounds/hooks/spawns/static collision are installed in a fixed order. Capacity and content validity are construction contracts rather than runtime rollback cases.
...
Concrete effects live beside it: `engine/display/effect/transform.h`, `engine/display/effect/zoom.h`, `engine/display/effect/turn.h`, `engine/display/effect/wave.h`, `engine/display/effect/bob.h`, `engine/display/effect/shake.h`, `engine/display/effect/shear.h` and `engine/display/effect/blink.h`; include only the effect you use. Every sprite owns `sprite.effect`; the effect keeps advancing even when sprite animation is stopped.
...
`UUIBlinkLabel` is initialized from a named `UUIBlinkLabelConfig` (`bounds`, `style`, text, interval, clear width and initial visibility) instead of a long positional argument list. Use the config form when composing blinking prompts.
...
Static object collisions are built once at load time. Dynamic collisions use an idle fast path: when no projectile or active hitbox can produce a query, the level only probes attack activity and avoids materializing every hurtbox. On query-producing frames, the previous dynamic tail is cleared once and boxes are refreshed/registered in the same pool traversal.

The `ULevelDefinition.resolve_hits` callback reads already detected pairs from the frame-local `level->collision.hits` container. It then applies game rules (damage, guard, reactions, deduplication), but does not recompute hitbox/hurtbox intersections itself.

Collision buffer capacities are composition contracts sized with the game configuration; the frame hot path does not add recovery branches for impossible under-capacity states.
...
If no timeout is required, simply call `unsigned_state_graph_tick()` and do not store a clock. `ULevelManager` exposes this distinction explicitly through `ULevelManagerMode`: graph mode owns/ticks a `UStateGraphClock`, while direct mode lets the higher-level application replace levels with `unsigned_level_manager_set()`.
...
In `src/game/demo_flow.c`, scene transitions are described by a rule table. `UDemoFlowContext.current` (`UDemoScene`) is the scene identity source of truth, and stable dependencies are bound once with `UDemoFlowConfig`. `UTimerPool` remains responsible for presentation durations/countdowns. Do not confuse this application flow with `UStateGraphClock`, which is only used to time a generic `UStateGraph`.
