...
Level rendering is split between `unsigned_game_instance_prepare_render()` during active display and the commit performed by `unsigned_game_instance_render()` immediately after VBlank. Legacy callers may still call `unsigned_game_instance_render()` directly; it falls back to prepare+commit.
...
6. update the Beat'Em Up camera from active players;
7. register/detect collisions;
8. call `resolve_hits` if hits exist;
9. tick effects;
10. tick cues;
...
`UCharacterShadow` remains a character component, but its responsibility is now limited to converting `UCharacter.height` into a generic scale. It owns a `UTransformEffect` bound to `shadow.sprite.effect`; rendering math and Neo Geo encoding stay outside `character_shadow.c`.
...
`URenderFrameStats` exposes the estimated VRAM words per priority, actor relocation count, actor sprite-column count and background column uploads. This instrumentation is available through `unsigned_game_instance_render_stats()`.
